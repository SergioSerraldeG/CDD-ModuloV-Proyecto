...
st.markdown(
    """<style>.stApp { background-color: #f0f2f6; }</style>""",
    unsafe_allow_html=True
)
st.title("🚗 AcciMex 🚑")
st.caption("🚨 Asistente de riesgo de accidentes viales en CDMX.")

st.subheader("Mapa de Riesgo por Alcaldía en Tiempo Real")
...
if prompt := st.chat_input(placeholder="Escribe una dirección, cruce o colonia..."):
    ...
    tools = [
        {"type": "function", "function": {
            ...
            "parameters": {"type": "object", "properties": {
                "direccion": {"type": "string", "description": "La dirección, ej: 'Paseo de la Reforma 222'."},
            }, "required": ["direccion"]},
        }},
        {"type": "function", "function": {
            "name": "encontrar_alcaldia_mayor_riesgo",
            "description": "Encuentra la alcaldía con el mayor riesgo de accidente en este momento. No requiere argumentos.",
            "parameters": {"type": "object", "properties": {}},
        }},
    ]
            ...
            if function_name == "obtener_coordenadas":
                with st.spinner("Obteniendo coordenadas y calculando riesgo..."):
                    ...
                    if "error" in coord_resp:
                        ...
                        risk_resp_str = obtener_riesgo_accidente(lat, lon)
