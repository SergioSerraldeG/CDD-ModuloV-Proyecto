...
if prompt := st.chat_input("🔍 Escribe una dirección, cruce o colonia…"):
    ...
    tools = [
        {
            ...
            "function": {
                ...
                "description": "Obtiene la latitud y longitud para una dirección.",
                "parameters": {
                    "type": "object",
                    "properties": {
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Encuentra la alcaldía con mayor riesgo actual.",
                "parameters": {
                    "type": "object",
                    "properties": {}
                }
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Calcula el riesgo usando clima y tráfico para un punto.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "lat": {"type": "number"},
    ...
    if tool_calls:
        ...
        if function_name == "obtener_coordenadas":
            ...
            with st.spinner("Obteniendo coordenadas y calculando riesgo..."):
                ...
                if "error" in coord_resp:
                    st.chat_message("assistant").error(coord_resp["error"])
                    st.stop()

                lat, lon = coord_resp["lat"], coord_resp["lon"]

                risk_resp_str = obtener_riesgo_especifico(lat, lon)
                risk_resp = json.loads(risk_resp_str)

                if "error" in risk_resp:
                    st.chat_message("assistant").error(risk_resp["error"])
                    st.stop()
                ...
                with st.chat_message("assistant"):
                    ...
                    st.write(f"📍 Ubicación analizada")

                    col1, col2, col3 = st.columns(3)

                    col1.metric(
                        label="🚨 Nivel de Riesgo",
                        value=f"{riesgo_val * 100:.1f}%"
                    )

                    col2.metric(
                        label="🚦 Congestión",
                        value=f"{risk_resp.get('congestion', 0) * 100:.0f}%"
                    )

                    col3.metric(
                        label="🌡️ Temperatura",
                        value=f"{risk_resp.get('temperatura', 0):.1f}°C"
                    )
                ...
                summary_messages = [
                    {"role": "system", "content": stronger_prompt},
                    {
                        "role": "user",
                        "content": (
                            f"Riesgo={riesgo_val:.2%}, "
                            f"Temperatura={risk_resp['temperatura']:.1f}°C, "
                            f"Clima={risk_resp['clima']}, "
                            f"Congestión={risk_resp['congestion']:.0%}. "
                            f"Da una recomendación de manejo."
                        )
                    }
                ]
        ...
        # ----------- CASO 2: ALCALDÍA CON MAYOR RIESGO -----------
            ...
            with st.spinner("Calculando riesgo para todas las alcaldías..."):
                ...
                final_text = result_data.get(
                    "mensaje",
                    "No se pudo determinar la alcaldía con mayor riesgo."
                )
